...
Ориентируйся на стиль наших ТЗ: конкретные цифры (суммы, количество мест), цепляющий заголовок, структура слайдов, живой язык без клише.
...
{extra_context}ПРИМЕРЫ НАШИХ ТЗ — ориентируйся на структуру, подачу и стиль:

ТЗ 1 — KNUS:
Заголовок: «Не только ЕНТ: теперь гранты получают и за спортивный потенциал»
Структура: название вуза → линейка грантов (название, для кого, что включает с ✓) → образовательные программы → контакты

ТЗ 2 — World Citizen:
Заголовок: «В 19 лет поступил в Испанию, хотя когда-то просто пришёл учить язык»
Структура: история студента → путь (шаги с ✔) → цитаты студента → возможности → CTA

ТЗ 3 — UniTap:
Заголовок: «Как всего за $20 подать в 500 вузов мира?»
Структура: что такое платформа → возможности списком → партнёры → форматы → как подать (шаги) → контакты

ТЗ 4 — Harmony:
Заголовок: «Школьники Астаны получили грант 5 млн тенге и поступили в топ-вузы мира»
Структура: результаты учеников (офферы, суммы, топ-вузы) → условия гранта → CTA с дедлайном

ТЗ 5 — TAU:
Заголовок: «Гранты, двойной диплом и карьерные возможности»
Структура: преимущества университета → виды грантов (каждый отдельным слайдом) → контакты
